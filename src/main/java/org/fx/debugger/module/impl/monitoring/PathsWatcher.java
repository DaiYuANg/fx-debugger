 ...
 */
package org.fx.debugger.module.impl.monitoring;
...
import static org.fx.debugger.module.impl.log.CSSFXLogger.logger;
...
public class PathsWatcher {
  ...
  public void monitor(Path directory, Path sourceFile, Runnable action) {
    if (watchService != null) {
      logger(PathsWatcher.class)
          .info(
              "registering action %d for monitoring %s in %s",
              System.identityHashCode(action), sourceFile, directory);
      ...
      actions.add(action);
      logger(PathsWatcher.class)
          .debug("%d CSS modification actions registered for file %s", actions.size(), sourceFile);
    } else {
              ...
              public void run() {
                ...
                while (true) {
                  ...
                  for (WatchEvent<?> event : key.pollEvents()) {
                    ...
                    logger(PathsWatcher.class)
                        .debug("'%s' change detected in directory %s", kind, directory);
                    ...
                    if (kind == StandardWatchEventKinds.ENTRY_MODIFY) {
                      ...
                      if (filesActions.containsKey(directory.toString())) {
                        logger(PathsWatcher.class)
                            .debug("file: %s was modified", modifiedFile.getFileName());
                        ...
                        if (filesAction.containsKey(modifiedFile.toString())) {
                          ...
                          logger(PathsWatcher.class)
                              .debug("%d CSS modification will be performed ", actions.size());

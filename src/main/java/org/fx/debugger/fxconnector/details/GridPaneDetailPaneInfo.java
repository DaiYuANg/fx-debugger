...
class GridPaneDetailPaneInfo extends DetailPaneInfo {
  ...
  protected void createDetails() {
    ...
    columnConstraintsDetail =
        addDetail("columns", "columnConstraints:", ValueType.GRID_CONSTRAINTS);

    rowListener =
        new ListChangeListener<RowConstraints>() {
          @Override
          public void onChanged(final Change<? extends RowConstraints> change) {
  ...
  protected void updateDetail(final String propertyName) {
    ...
    if (all || propertyName.equals("hgap") || propertyName.equals("vgap")) {
      gapDetail.setValue(
          gridpane != null ? (gridpane.getHgap() + " / " + gridpane.getVgap()) : "-");
      gapDetail.setIsDefault(
          gridpane == null || (gridpane.getHgap() == 0 && gridpane.getVgap() == 0));
      gapDetail.setSimpleSizeProperty(
          gridpane != null ? gridpane.hgapProperty() : null,
          gridpane != null ? gridpane.vgapProperty() : null);
      if (!all) gapDetail.updated();
      if (!all) return;
    }
    if (all || propertyName.equals("alignment")) {
      ...
      alignmentDetail.setEnumProperty(
          gridpane != null ? gridpane.alignmentProperty() : null, Pos.class);
      if (!all) alignmentDetail.updated();
      if (!all) return;
    }
    if (all || propertyName.equals("gridLinesVisible")) {
      gridLinesVisibleDetail.setValue(
          gridpane != null ? Boolean.toString(gridpane.isGridLinesVisible()) : "-");
      gridLinesVisibleDetail.setIsDefault(gridpane == null || !gridpane.isGridLinesVisible());
      gridLinesVisibleDetail.setSimpleProperty(
          gridpane != null ? gridpane.gridLinesVisibleProperty() : null);
      if (!all) gridLinesVisibleDetail.updated();
      if (!all) return;
    }
  ...
  private void updateColumnConstraints() {
    ...
    final ObservableList<ColumnConstraints> columns =
        gridpane != null ? gridpane.getColumnConstraints() : null;
    ...
    if (columns != null) {
      for (int rowIndex = 1; rowIndex <= columns.size(); rowIndex++) {
        ...
        columnConstraintsDetail.add(
            cc.getPercentWidth() != -1 ? f.format(cc.getPercentWidth()) : "-",
            colIndex++,
            rowIndex);
  ...
  private void updateRowConstraints() {
    ...
    final ObservableList<RowConstraints> rows =
        gridpane != null ? gridpane.getRowConstraints() : null;
    ...
    if (rows != null) {
      for (int rowIndex = 1; rowIndex <= rows.size(); rowIndex++) {
        ...
        rowConstraintsDetail.add(
            rc.getPercentHeight() != -1 ? f.format(rc.getPercentHeight()) : "-",
            colIndex++,
            rowIndex);

 ...
 */
package org.fx.debugger.fxconnector.details;
...
import org.fx.debugger.fxconnector.ConnectorUtils;
import org.fx.debugger.fxconnector.StageID;
import org.fx.debugger.fxconnector.event.FXConnectorEventDispatcher;
...
class NodeDetailPaneInfo extends DetailPaneInfo {
  ...
  NodeDetailPaneInfo(final FXConnectorEventDispatcher dispatcher, final StageID stageID) {
    ...
    transformListener =
        new ListChangeListener<Transform>() {
          @Override
  ...
  protected void createDetails() {
    ...
    boundsInParentDetail =
        addDetail("boundsInParent", "boundsInParent:", Detail.LabelType.BOUNDS_PARENT);
  ...
  private void fillPseudoStates(final Node node) {
    ...
    //                        final String pseudoClass = name.substring(0,
    // name.indexOf("_PSEUDOCLASS_STATE")).toLowerCase();
    //                        pseudoStates.put(lvalue, pseudoClass);
    //                    } catch (final IllegalArgumentException e) {
    //                        ScenicViewlog.atError().log(e);
  ...
  protected void updateAllDetails() {
    ...
    if (node != null) {
      ...
      maxSizeDetail.setValue(
          (maxw >= Double.MAX_VALUE ? "MAXVALUE" : f.format(maxw))
              + " x "
              + (maxh >= Double.MAX_VALUE ? "MAXVALUE" : f.format(maxh)));
    } else {
  ...
  protected void updateDetail(final String propertyName) {
    ...
    if (all || propertyName.equals("styleClass")) {
      ...
      styleClassDetail.setSerializer(
    ...
    if (all || propertyName.equals("effect")) {
      ...
      if (!all) effectDetail.updated();
      if (!all) return;
    }
    if (all || propertyName.equals("layoutBounds")) {
      layoutBoundsDetail.setValue(
          node != null ? ConnectorUtils.boundsToString(node.getLayoutBounds()) : "-");
      layoutBoundsDetail.setIsDefault(node == null);
      if (!all) layoutBoundsDetail.updated();
      if (!all) return;
    }
    if (all || propertyName.equals("transforms")) {
      ...
      if (!all) transformsDetail.updated();
      if (!all) return;
    }
    if (all || propertyName.equals("scaleX") || propertyName.equals("scaleY")) {
      scaleXYDetail.setValue(
          node != null ? f.format(node.getScaleX()) + " x " + f.format(node.getScaleY()) : "-");
      scaleXYDetail.setIsDefault(
          node == null || (node.getScaleX() == 1.0 && node.getScaleY() == 1.0));
      scaleXYDetail.setSimpleSizeProperty(
          node != null ? node.scaleXProperty() : null, node != null ? node.scaleYProperty() : null);
      if (!all) scaleXYDetail.updated();
      if (!all) return;
    }
    ...
    if (all || propertyName.equals("clip")) {
      ...
      clipDetail.setValue(
          node != null
              ? clip != null
                  ? (clip.getClass().getSimpleName()
                      + ":"
                      + ConnectorUtils.boundsToString(clip.getBoundsInLocal()))
                  : "null"
              : "-");
      clipDetail.setIsDefault(clip == null);
      if (!all) clipDetail.updated();
      if (!all) return;
    }
    if (all || propertyName.equals("layoutX") || propertyName.equals("layoutY")) {
      layoutXYDetail.setValue(
          node != null ? f.format(node.getLayoutX()) + "," + f.format(node.getLayoutY()) : "-");
      layoutXYDetail.setIsDefault(
          node == null || (node.getLayoutX() == 0 && node.getLayoutY() == 0));
      layoutXYDetail.setSimpleSizeProperty(
          node != null ? node.layoutXProperty() : null,
          node != null ? node.layoutYProperty() : null);
      if (!all) return;
    }
    if (all || propertyName.equals("translateX") || propertyName.equals("translateY")) {
      translateXYDetail.setValue(
          node != null
              ? f.format(node.getTranslateX()) + "," + f.format(node.getTranslateY())
              : "-");
      translateXYDetail.setIsDefault(
          node == null || (node.getTranslateX() == 0 && node.getTranslateY() == 0));
      translateXYDetail.setSimpleSizeProperty(
          node != null ? node.translateXProperty() : null,
          node != null ? node.translateYProperty() : null);
      if (!all) translateXYDetail.updated();
      if (!all) return;
    }
    if (all || propertyName.equals("boundsInParent")) {
      final String bounds =
          node != null ? ConnectorUtils.boundsToString(node.getBoundsInParent()) : "-";
      ...
      if (!bounds.equals(boundsInParentDetail.getValue())) {
        ...
        if (!all) boundsInParentDetail.updated();
      }
    }

 ...
 */
package org.fx.debugger.fxconnector.gui;
...
public class ComponentHighLighter extends Group {
  ...
  public ComponentHighLighter(
      final SVNode node, final double width, final double height, final Bounds bounds) {
    if (width == -1) {
      ...
      label.setText(
          "x:"
              + ConnectorUtils.format(bounds.getMinX())
              + " y:"
              + ConnectorUtils.format(bounds.getMinY())
              + "\nw:"
              + ConnectorUtils.format(rect.getWidth())
              + " h:"
              + ConnectorUtils.format(rect.getHeight()));

 ...
 */
package org.fx.debugger.fxconnector.remote;
...
import org.fx.debugger.fxconnector.*;
...
import org.fx.debugger.utils.Platform;
import org.fx.debugger.utils.RMIUtils;
...
class RemoteConnectorImpl extends UnicastRemoteObject implements RemoteConnector, FXConnector {
  ...
  public void onAgentStarted(final int port) {
    log.atInfo().log("Remote agent started on port:" + port);
  ...
  private void addStages(
    ...
    for (int i = 0; i < ids.length; i++) {
      log.atInfo().log("RemoteApp connected on:" + port + " stageID:" + ids[i]);
      final int cont = i;
      impl.getStages()
          .add(
              new StageController() {
  ...
  public List<AppController> connect() {
    ...
    log.atTrace().log(machines.size() + " JavaFX applications found");
    ...
    for (final VirtualMachine machine : machines) {
      ...
      if (applications.containsKey(temp.id())) {
        ...
        try {
          ...
          log.atError().log(e.getMessage(), "Failure connecting to machine.");
  ...
  private void loadAgent(final VirtualMachine machine, final File agentFile) {
    ...
    machine.loadAgent(
        agentFile.getAbsolutePath(), port + ":" + this.port + ":" + machine.id() + ":");
    machine.detach();
  }
  ...
  private List<VirtualMachine> getRunningJavaFXApplications() {
    ...
    log.atTrace().log("Number of running Java applications found: " + machines.size());
    ...
    for (int i = 0; i < machines.size(); i++) {
      ...
      try {
        ...
        log.atTrace()
            .log("Obtaining properties for Java application with PID:" + virtualMachine.id());
        ...
        //                log.atInfo().log("JVM:" + virtualMachine.id() + " detection finished");
  ...
  private File findAgent() {
    ...
    try {
      URL url =
          RuntimeAttach.class.getResource(
              "/org/fx/debugger/fxconnector/remote/RuntimeAttach.class");
      ...
      log.atError().log(e.getMessage(), "Attempting to get agent jar.");
    }
    ...
    if (tempf == null) {
      log.atInfo().log("Error: Unable to find agent jar file. Exiting Scenic View.");
      System.exit(-1);
    } else {
      log.atInfo().log("Loading agent from: " + tempf.getAbsolutePath());
    }
    return tempf;
  }
}

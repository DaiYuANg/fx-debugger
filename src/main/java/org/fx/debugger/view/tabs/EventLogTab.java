 ...
 */
package org.fx.debugger.view.tabs;
...
import org.fx.debugger.utils.DisplayUtils;
import org.fx.debugger.view.ContextMenuContainer;
import org.fx.debugger.view.ScenicViewGui;
import org.fx.debugger.view.control.FilterTextField;
import org.fx.debugger.view.dialog.InfoBox;
...
public class EventLogTab extends Tab implements ContextMenuContainer {
  ...
  private Node buildUI() {
    ...
    eventTypeCol.setCellValueFactory(
        new PropertyValueFactory<ScenicViewEvent, String>("eventType"));
    ...
    eventValueCol.setCellValueFactory(
        new PropertyValueFactory<ScenicViewEvent, String>("eventValue"));
    ...
    moreInfoCol.setCellValueFactory(
        new PropertyValueFactory<ScenicViewEvent, StackTraceElement[]>("stackTrace"));
    moreInfoCol.setCellFactory(
        new Callback<
            TableColumn<ScenicViewEvent, StackTraceElement[]>,
            TableCell<ScenicViewEvent, StackTraceElement[]>>() {

          @Override
          public TableCell<ScenicViewEvent, StackTraceElement[]> call(
              final TableColumn<ScenicViewEvent, StackTraceElement[]> arg0) {
            final TableCell<ScenicViewEvent, StackTraceElement[]> cell =
                new TableCell<ScenicViewEvent, StackTraceElement[]>() {
                  {
                    setId("");
        ...
        .addListener(
            new ChangeListener<Boolean>() {
              ...
              public void changed(
                ...
                if (newValue)
                  scenicView.setStatusText(
                      "Type any text for filtering (logical expressions NOT AND and OR are supported, example NOT MOUSE_MOVED AND TilePane)");
                else scenicView.clearStatusText();
              }
            });
    idFilterField
        .getTextField()
  ...
  public void setSelectedNode(final SVNode selectedNode) {
    ...
    if (!activateTrace.isSelected()) {
      ...
      selectedNodeLabel.setText(
          "Filtering from current selection: " + selectedNode.getExtendedId());
    } else {
  ...
  private boolean validForFilter(final ScenicViewEvent event) {
    if (idFilterField.getText().equals("")) return true;
    final String lower = idFilterField.getText().toLowerCase();
    final String[] unparsed = lower.split(" ");
    final String eventData =
        event.eventType.toLowerCase() + event.eventValue.toLowerCase() + event.source.toLowerCase();
    boolean valid = true;
    boolean and = true;
    boolean not = false;
    for (int i = 0; i < unparsed.length; i++) {
      if (unparsed[i].equals("and") && i < unparsed.length - 1) {
        ...
        if (and && not) valid &= !actualValid;
        else if (and) valid &= actualValid;
        else if (!and && not) valid |= !actualValid;
        else valid |= actualValid;
        and = true;
        not = false;
      }
    }
  ...
  public class ScenicViewEvent {
    ...
    public String toString() {
      return "Event [source="
          ...
          + ", eventType="
          + eventType
          + ((eventValue != null && !eventValue.equals("")) ? (", eventValue=" + eventValue) : "")
          + ", moment="
          + moment
          + "]";
    }

...
public class Tile3D extends Box {
  ...
  public Tile3D(
    ...
    getTransforms()
        .add(
            new Translate(
                bounds3D.getMinX() + bounds3D.getWidth() / 2,
  ...
  public final void snapshot() {
    ...
    if (layoutBounds.getWidth() > 0 && layoutBounds.getHeight() > 0) {
      writableImage =
          new WritableImage((int) layoutBounds.getWidth(), (int) layoutBounds.getHeight());
      SnapshotParameters snapshotParameters = new SnapshotParameters();

      // Hide children
      if (node2d.getImpl() instanceof Parent) {
        ObservableList<Node> childrenUnmodifiable =
            ((Parent) node2d.getImpl()).getChildrenUnmodifiable();
  ...
  private Bounds localetoRoot(SVNode sv) {
    ...
    return new BoundingBox(
        node.getMinX() - root.getMinX(),
        node.getMinY() - root.getMinY(),
        node.getWidth(),
        node.getHeight());
  }

...
import org.fx.debugger.utils.PropertiesUtils;

/**
 * Main class for 3D display TODO: 2D to 3D Miss: a snapshot parameter to only capture container and
 * not descendants (RFE for JDK9). Replace Tile3D by a Mesh with only one textured face.
 */
public class ThreeDOM implements ITile3DListener {
  ...
  @FXML Label overTileText;
  @FXML Button reload;
  @FXML TitledPane controls;
  @FXML Accordion accordion;
  @FXML BorderPane subSceneContainer;
  @FXML ColorPicker colorPicker;
  @FXML Button defaultBackgroundColor;
  @FXML Slider spaceSlider;

  private static final String STYLESHEETS =
      ThreeDOM.class.getResource("threedom.css").toExternalForm();
  ...
  public Parent createContent(SVNode root2D) throws Exception {
    ...
    camera.getTransforms().addAll(rotateY, rotateX, translateCamera);
    ...
    ZoomSupport zoomSupport =
        new ZoomSupport(
            ...
            translateCamera.zProperty(),
            translateCamera.xProperty(),
            1);
    ...
    spaceSlider
        ...
        .addListener(
            (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
              ...
              childrenUnmodifiable.stream()
                  .forEach(
                      (n) -> {
                        double z = ((Tile3D) n).getTranslateZ();
                        ((Tile3D) n)
                            .setTranslateZ(((Tile3D) n).get3Depth() * -newValue.doubleValue());
                      });
            });
    //
  ...
  private Tile3D from2Dto3D(SVNode root2D, Group root3D, double depth) {
    ...
    // Currently, only work with internal usage. Remote is not yet possible, mainly due to the
    // node.snapshot() calls
    if (ConnectorUtils.nodeClass(root2D).contains("SVRemoteNodeAdapter")) {
      Label label =
          new Label(
              "The 3D view of ScenicView is only accessible when you invoke the tool from your source code. Remote access is not yet available.");
  ...
  private void setDepth(double depth, Group world) {
    ...
    childrenUnmodifiable.stream()
        .filter((child) -> (child instanceof Tile3D))
        .forEach(
            (Node child) -> {
  ...
  public void setSelectedTile(Tile3D tile) {
    ...
    if (currentSelectedNode != null) {
      ...
      Platform.runLater(
          () -> {
            ((Tile3D) currentSelectedNodeToUnselect)
                .snapshot(); // In case of changes on 2D node after edit
          });
    }
    currentSelectedNode = tile;
    ((PhongMaterial) tile.getMaterial())
        .setDiffuseColor(Color.YELLOW); // Simulate SV selection color
  }
  ...
  void applyColor(Color color) {
    ...
    subSceneContainer.setBackground(

...
import org.fx.debugger.fxconnector.event.*;
import org.fx.debugger.fxconnector.node.SVNode;
import org.fx.debugger.model.Persistence;
import org.fx.debugger.update.AppsRepository;
import org.fx.debugger.update.UpdateStrategy;
import org.fx.debugger.utils.DisplayUtils;
import org.fx.debugger.view.control.FilterTextField;
import org.fx.debugger.view.dialog.AboutBox;
import org.fx.debugger.view.dialog.HelpBox;
import org.fx.debugger.view.tabs.*;
...
public class ScenicViewGui {
  ...
  public static final String STYLESHEETS =
      ScenicViewGui.class.getResource("scenicview.css").toExternalForm();
  ...
  private void buildUI() {
    ...
    tabPane
        .getTabs()
        .addAll(detailsTab, eventsTab, /*animationsTab,*/ javadocTab, threeDOMTab, cssfxTab);
    // /3Dom
  ...
  private void buildFiltersBox() {
        ...
        new NodeFilter() {
          ...
          public boolean accept(final SVNode node) {
            ...
            return node.getId() != null
                && node.getId().toLowerCase().indexOf(idFilterField.getText().toLowerCase()) != -1;
          }
    ...
    activeNodeFilters.add(
        new NodeFilter() {
          ...
          @Override
          public boolean accept(final SVNode node) {
            if (classNameFilterField.getText().equals("")) return true;

            // Allow reduces or complete className
            return node.getNodeClass()
                    .toLowerCase()
                    .indexOf(classNameFilterField.getText().toLowerCase())
                != -1;
          }
  ...
  private void buildMenuBar() {
    ...
    //        final CheckMenuItem showRuler = buildCheckMenuItem("Show Ruler", "Show ruler in the
    ...
    //
    //            @Override public void changed(final ObservableValue<? extends Color> arg0, final
  ...
  //                        } else if (forced) {
  //                            new InfoBox("Version check", "You already have the latest version of
  ...
  private void closeApps() {
    for (final Iterator<AppController> iterator = appRepository.getApps().iterator();
        iterator.hasNext(); ) {

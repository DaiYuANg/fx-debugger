...
import org.fx.debugger.view.tabs.DetailsTab;
import org.fx.debugger.view.tabs.EventLogTab;
import org.fx.debugger.view.tabs.JavaDocTab;
import org.fx.debugger.view.tabs.ThreeDOMTab;
...
public class ScenegraphTreeView extends TreeView<SVNode> {
        ...
        .addListener(
            (ov, oldValue, newValue) -> {
              if (!blockSelection) {
                ...
                setSelectedNode(
                    selected != null && !(selected.getValue() instanceof SVDummyNode)
                        ? selected
                        : null);
              }
            });
  ...
  private EventHandler<ActionEvent> forceExpandCollapse(
  ...
  TreeItem<SVNode> findStageForNode(final TreeItem<SVNode> item) {
    ...
    if (item.getValue() instanceof SVDummyNode
        && ((SVDummyNode) item.getValue()).getNodeType() == NodeType.STAGE) {
      return item;
    } else {
    ...
    if (apps.size() == 1) {
      ...
      if (app.getChildren().size() == 0
          || (app.getChildren().size() == 1
              && app.getChildren().get(0).getValue().equals(stageRoot.getValue()))) {
  ...
  private TreeItem<SVNode> getTreeItem(final SVNode node) {
    ...
    if (item == null) {
      for (final Iterator<SVNode> iterator = treeViewData.keySet().iterator();
          iterator.hasNext(); ) {

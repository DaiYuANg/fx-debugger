...
import org.fx.debugger.fxconnector.remote.RemoteApplication;
import org.fx.debugger.fxconnector.remote.RemoteConnector;
...
public class RMIUtils {
  ...
  private static final RemoteConnector findScenicView(
      final String serverAdress, final int serverPort) throws Exception {
  ...
  private static final RemoteApplication findApplication(
      final String serverAdress, final int serverPort) throws Exception {
    final Registry registry = LocateRegistry.getRegistry(serverAdress, serverPort);
    // look up the remote object
  ...
  public static final void bindScenicView(final RemoteConnector view, final int port)
    ...
    new Thread(REMOTE_CONNECTOR + ".Finder") {
      ...
      public void run() {
        ...
        while (scenicView == null) {
          try {
            log.atInfo().log("Finding " + REMOTE_CONNECTOR + " connection for agent...");

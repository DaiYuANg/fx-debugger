...
import org.fx.debugger.fxconnector.Configuration;
...
public class SCUtils {
  ...
  public void removeScenicViewComponents(final Node target) {
  ...
  public Bounds toSceneBounds(
    ...
    if (parent != null) {
      ...
      final Point2D pt =
          overlayParent.sceneToLocal(
              node.getParent().localToScene(bounds.getMinX(), bounds.getMinY()));
  ...
  public void addToNode(final Parent parent, final Node node) {
  ...
  public void removeFromNode(final Parent parent, final Node node) {
    if (parent instanceof Group) {
  ...
  public Node findNode(final Node target, final int nodeUniqueID) {
  ...
  public final boolean isNormalNode(final Node node) {
    return ConnectorUtils.isNormalNode(node);
  }

  public Node getHoveredNode(
      final Configuration configuration, final Node target, final double x, final double y) {
    ...
    if (target.contains(localPoint)
        && ((!configuration.isIgnoreMouseTransparent()
                || !ConnectorUtils.isMouseTransparent(target))
            && ConnectorUtils.isNodeVisible(target))) {
      return target;
    }
  ...
  public Parent findFertileParent(final Parent p) {

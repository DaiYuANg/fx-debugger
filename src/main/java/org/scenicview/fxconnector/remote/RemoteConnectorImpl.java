 ...
 */
package org.scenicview.fxconnector.remote;
...
import org.scenicview.fxconnector.AppControllerImpl;
...
class RemoteConnectorImpl extends UnicastRemoteObject implements RemoteConnector, FXConnector {
  ...
  public List<AppController> connect() {
    ...
    log.atInfo().log(machines.size() + " JavaFX applications found");
  ...
  private void loadAgent(final VirtualMachine machine, final File agentFile) {
    ...
    machine.loadAgent(
        agentFile.getAbsolutePath(),
        Integer.toString(port) + ":" + this.port + ":" + machine.id() + ":");
    machine.detach();
  }
  ...
  private List<VirtualMachine> getRunningJavaFXApplications() {
    ...
    log.atInfo().log("Number of running Java applications found: " + machines.size());
    ...
    for (int i = 0; i < machines.size(); i++) {
      ...
      try {
        ...
        log.atInfo().log(
            "Obtaining properties for Java application with PID:" + virtualMachine.id());
  ...
  private File findAgent() {
    ...
    try {
      URL url =
          RuntimeAttach.class.getResource("/org/scenicview/fxconnector/remote/RuntimeAttach.class");
